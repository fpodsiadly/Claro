...
def handler(request):
    ...
    """
    # Owijamy całą funkcję w try-except, aby zawsze zwrócić poprawny JSON
    try:
        ...
        if request.get("method", "").upper() == "OPTIONS":
            ...
            return {
                "statusCode": 200,
                "headers": headers,
                "body": "{}"  # Pusty obiekt JSON zamiast pustego stringa
            }
        ...
        # Przechwytujemy WSZYSTKIE wyjątki i zwracamy poprawny JSON
        error_id = hashlib.md5(str(time.time()).encode()).hexdigest()[:8]
        error_details = traceback.format_exc()
        logger.error(f"[{error_id}] Krytyczny błąd podczas przetwarzania żądania: {str(e)}")
        logger.error(f"[{error_id}] Szczegóły błędu: {error_details}")
        
        # Zawsze zwracamy poprawny JSON, nawet w przypadku krytycznych błędów
        headers = {
            "Content-Type": "application/json",
            "Access-Control-Allow-Origin": "*",
            "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
